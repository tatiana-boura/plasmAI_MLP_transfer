...
% Pressure = dataTable(:,2);
% Power = dataTable(:,1);
%clear dataTable;
%plot the original data

%plot(Power, Pressure, 'bo');
...
center_Pressure = mean(dataTable(:,2));
center_Power = mean(dataTable(:,1));

%Corners 
x_min = center_Power - side_length_power/2;
x_max = center_Power + side_length_power/2;

y_min = center_Pressure - side_length_pressure/2;
y_max = center_Pressure + side_length_pressure/2;
...
writematrix(test_data, 'test_data_no_head.csv', 'Delimiter', ';');
writematrix(train_data, 'train_data_no_head.csv', 'Delimiter', ';');
